...
func (this *TaskCreateForm) Valid(v *validation.Validation) {
	...
	v.MinSize(this.Name, 2, "name.name").Message("必须在%d~%d之间", 2, 32)
	v.MaxSize(this.Name, 32, "name.name").Message("必须在%d~%d之间", 2, 32)

	v.MinSize(this.Worker, 2, "worker.worker").Message("必须在%d~%d之间", 2, 32)
	v.MaxSize(this.Worker, 32, "worker.worker").Message("必须在%d~%d之间", 2, 32)
...
func (this *TaskModifyForm) Valid(v *validation.Validation) {
	...
	if orm.NewOrm().Read(&task) == nil {
		if this.User.IsSupper || this.User.Id == task.CreateUser {
			this.Task = &task
		} else {
