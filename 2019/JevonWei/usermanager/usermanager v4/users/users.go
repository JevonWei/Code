...
type Users struct {
	...
	Birthday time.Time `json:brithday, string`
...
// json序列化用户数据
func SaveUser_json(users map[int]Users) {
	os.Mkdir(json_dir, 0644)
	//json_file := filepath.Join(json_dir, (strconv.FormatInt(time.Now().Unix(), 10) + ".user.json"))
	json_file := filepath.Join(json_dir, ".user.json")

	// 将user文件重命名
	if _, err := os.Stat(json_file); err == nil {
		os.Rename(json_file, filepath.Join(json_dir, (strconv.FormatInt(time.Now().Unix(), 10)+".user.json")))
		log.Printf("%s文件已备份\n", json_file)
	}

	// 仅保留三个用户历史文件
	if names, err := filepath.Glob(json_dir + "/*.user.json"); err == nil {
...
func LoadUser_json() map[int]Users {
	users := map[int]Users{}
	json_file := filepath.Join(json_dir, ".user.json")
...
func Inputuser(num int) {
	...
	for _, user := range LoadUser_json() {
