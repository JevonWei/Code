...
func (m UserMap) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	for k, v := range m {
		e.Encode(xmlMapEntry{XMLName: xml.Name{Local: strconv.Itoa(k)}, Users: v})
	}

	return e.EncodeToken(start.End())
}

func (m *UserMap) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	*m = UserMap{}
	for {
		var e xmlMapEntry
		...
		i, _ := strconv.Atoi(e.XMLName.Local)
		(*m)[i] = e.Users
	}
	return nil
}
...
func LoadUser() map[int]User {
	...
	if file, err := os.Open(UserFile); err == nil {
		...
		if err == nil {
			if err = xml.Unmarshal(bytes, (*UserMap)(&users)); err != nil {
				fmt.Println(err)
			}
		}
	} else {
...
func StoreUser(users map[int]User) {
	...
	if _, err := os.Stat(UserFile); err == nil {
		os.Rename(UserFile, strconv.FormatInt(time.Now().Unix(), 10)+".users.xml")
		fmt.Println("file")
	}

	// 超过三份以上的文件删除
	if names, err := filepath.Glob("*.users.xml"); err == nil {
	...
	if file, err := os.Create(UserFile); err == nil {
		defer file.Close()
		fmt.Println(users)
		bytes, err := xml.Marshal(UserMap(users))
